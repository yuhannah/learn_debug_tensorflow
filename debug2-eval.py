...
for i in range(1000):
    ...
    print("run: ", cost)
    print("eval: ", loss.eval(session=sess, feed_dict={x: batch_xs, y_: batch_ys}))
    costs.append(cost)
...
print("eval: ", loss.eval(session=sess, feed_dict={x: mnist.test.images, y_: mnist.test.labels}))
