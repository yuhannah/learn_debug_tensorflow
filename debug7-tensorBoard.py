...
def train():
    # Import data
    mnist = input_data.read_data_sets("mnist", fake_data=FLAGS.fake_data, one_hot=False)
    ...
    def nn_layer(input_tensor, input_dim, output_dim, layer_name, act=tf.nn.relu):
        ...
        with tf.name_scope(layer_name):
            # This Variable with hold the state of the weights for the layer
    ...
    # Do not apply softmax activation yet, see blow.
    ...
    for i in range(FLAGS.max_steps):
        if i % 10 == 0:  # Record summaries and test-set accuracy
        ...
        else:  # Record train set summaries, and stats
            if i % 100 == 99:  # Record execution stats
                ...
                # print(sess.run(y,feed_dict=feed_dict(True)))
                # print(sess.run(y_,feed_dict=feed_dict(True)))
                train_writer.add_summary(summary, i)
    train_writer.close()
    test_writer.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_dir', type=str, default=os.path.join('./', 'mnist/input_data'),
                        help='Directory for storint input data')
    parser.add_argument('--log_dir', type=str, default=os.path.join('./', 'mnist/logs/mnist_with_summaries'),
